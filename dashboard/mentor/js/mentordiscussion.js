    ...
    async function fetchMentorDashboard(user_id) {
        try {
    ...
    // async function studentLecture(m_id) {
    //     try {
...
//     try {
//         const response = await fetch('https://learning-u7aw.onrender.com/api/discussion', {
//             method: 'POST',
//             headers: {
//                 'Content-Type': 'application/json'
//             },
...
function displaySubdiscussion(data, discussionId) {
    ...
    // Sort subdiscussions array: mentor's subdiscussion first, then by likes
    const sortedSubdiscussions = data.subdiscussions.sort((a, b) => {
        // Mentor's subdiscussion comes first
        if (a.user_role === 'mentor' && b.user_role !== 'mentor') return -1;
        if (a.user_role !== 'mentor' && b.user_role === 'mentor') return 1;

        // If both are mentor or not, sort by likes
        return b.likes - a.likes;
    });

    // Iterate over each sorted subdiscussion and create HTML for each card
    sortedSubdiscussions.forEach((subdiscussion, subIndex) => {
...
async function viewDiscussion(c_id) {
    try {
        const response = await fetch('https://learning-u7aw.onrender.com/api/viewdiscussion', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            const reversedDiscussions = responseData.discussions.reverse();
            ...
            accordionItems.forEach((item) => {
                item.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        const response = await fetch('https://learning-u7aw.onrender.com/api/viewsubdiscussion', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
...
async function likediscussion(discussionId) {
    try {
        ...
        // Send a POST request to the API endpoint
        const response = await fetch('https://learning-u7aw.onrender.com/api/discussionlike', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function likesubdiscussion(subdiscussionId) {
    try {
        ...
        // Send a POST request to the API endpoint
        const response = await fetch('https://learning-u7aw.onrender.com/api/subdiscussionlike', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
